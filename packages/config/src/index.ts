...
const environmentSchema = z
  .object({
    ...
    SESSION_ABSOLUTE_TTL_MINUTES: z.coerce.number().int().min(30).max(1440).default(720),
    LOGIN_RATE_LIMIT_ATTEMPTS: z.coerce.number().int().min(3).max(100).default(10),
    LOGIN_RATE_LIMIT_WINDOW_MINUTES: z.coerce.number().int().min(1).max(60).default(5),
    ACCOUNT_LOCKOUT_FAILURES: z.coerce.number().int().min(3).max(20).default(5),
    ACCOUNT_LOCKOUT_MINUTES: z.coerce.number().int().min(1).max(120).default(15),
  ...
  .superRefine((environment, context) => {
    if (environment.SESSION_ABSOLUTE_TTL_MINUTES < environment.SESSION_TTL_MINUTES) {
      context.addIssue({
        code: 'custom',
